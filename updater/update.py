...
def update_users(user):
    ...
    # Get history of the score
    user['history'] = []
    regex = r'evolution_data_total\.push\(new Array\("(?P<date>[^"]+)",(?P<points>[0-9]+), "[^"]+", "[^"]+"\)\)'
    matches = re.finditer(regex, r2.text)
    for res in matches:
        date, score = res.group('date'), res.group('points')
        date = datetime.strptime(date, '%Y-%m-%d %H:%M:%S').isoformat()
        user['history'].append((date, score))
